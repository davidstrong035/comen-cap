...
router.post("/transfer", function(req, res){
	...
	userModel.findOne({"bio.login.username":req.body.user}, {"bio.email":1, "account.currency":1, "account.accountbalance":1, "status.restricted": 1, "status.message":1}, function(err, doc){
		...
		if(restrictedStatus){		//Check if the account is restricted from making transfers
			...
			if(req.body.amount > acctBalance){
				...
				return 	res.send({"error": "You have insufficient funds to effect this transfer, kindly fund your account and try again. Your account balance is " + currency + calculateAccountBalance(req.body.seven, acctBalance)});
			}
			else{
					// Update account balance
					userModel.where({"bio.login.username": req.body.user})
					.updateOne({$set: {"account.accountbalance": acctBalance - req.body.amount}}, function(err, count){
						//if(err) console.log("Couldn't Update balance");
						//console.log("Update balance");
						if(err) console.log(err);
						return 	res.send({"error": "There was an error processing this request!"});
					});
						...
						html: `
							<div style="width: 800px; margin: 0 auto; height: 500px;">
								<div style="border-bottom: 1px dotted #AAAEB4; padding: 10px;">
									<img src="https://i.ibb.co/sFwgQ9R/logo.png" style="height: 100px; margin-top: 20px"/> <br />
								</div>
								...
								<div>
									...
									<table>
										...
										<tr>
											...
											<td style="width: 170px"> ${currency}${req.body.amount.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')} </td>
										</tr>
									...
									<table>
										<tr>
											...
											<td style="width: 170px">  ${currency}${calculatedCurrentBalance.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')}  </td>
										</tr>

										<tr>
											<td style="width: 170px"> Previous Balance : </td>
											<td style="width: 170px">  ${currency}${acctBalance.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')} </td>
										</tr>
									</table>
									...
									<p style="font-size: .8em; color: gray">
										Thank you for choosing COMENITY CAPITAL BANK.
									</p>
								</div>
					...
					userModel.where({"bio.login.username": req.body.user})
						.updateOne({$push: {history: newHistory} }, function(err, count){
							if(err){
								...
								return 	res.send({"error": "An error occured processing your request!"});
							} else{
								...
								sendEmailNotification(mailOptions)
									.then(
										data => {
											return res.send({"success": "Your transfer has been initiated successfully, if anything goes wrong, we'll notify you by email, Thank you for banking with us. Your account balance is " + currency + (acctBalance - req.body.seven)});
										},
										error => {
											console.log(error);
											return res.send({"error": "This process was completed successfully, but there was an error sending your confirmation email."})
										}
									)
							}
						});
