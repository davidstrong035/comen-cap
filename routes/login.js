  ...
  userModel.find({ "bio.login.username": uname }, function (err, user) {
    if (err) {
      ...
      if (user.length > 0) {
        //console.log(user);
        if (
          uname == user[0].bio.login.username &&
          pwd == user[0].bio.login.password
        ) {
          res.send({
            username: uname,
            password: pwd,
            name: user[0].bio.name,
            time: user[0].timestamp,
          });
          console.log("User found");
        } else {
          res.send({
            error:
              "Your username and/or password isn't correct. Please try again",
          });
        }
      } else {
